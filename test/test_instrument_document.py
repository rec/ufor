...
from ufor.codec import document_schema, document_toml, parse_document
from ufor.samples import playback, processing, selection
from ufor.samples.controls import Control
from ufor.samples.instrument import Instrument, InstrumentDocument
...
def test_sfz_conformance_requires_only_text_and_supplied_asset_facts() -> None:
    ...
    result = sfz.compile(
        ...
        output_timebase=Timebase(id='output', rate=Rate(numerator=48000)),
...
def test_native_instrument_round_trips_through_the_common_codec() -> None:
    document = InstrumentDocument.model_validate(fixture())
    assert parse_document(document_toml(document)) == document
    assert (
        InstrumentDocument.model_validate_json(document.model_dump_json()) == document
    )
...
def test_documented_native_example_is_complete() -> None:
    ...
    assert isinstance(document, InstrumentDocument)
    assert parse_document(document_toml(document)) == document
...
def test_whole_envelope_overrides_and_playback_inheritance_round_trip() -> None:
    ...
    document = InstrumentDocument.model_validate(raw)
    restored = parse_document(document_toml(document))
...
def test_sfz_reports_general_envelopes_and_custom_channel_maps_as_unsupported() -> None:
    ...
    result = sfz.write(InstrumentDocument.model_validate(raw))
