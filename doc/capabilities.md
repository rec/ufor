...
| --- | --- | --- | --- |
| Scores and libraries | Tagged score union, metadata, references, selectors, presets, local interfaces | JSON/TOML interchange, schema generation, explicit local library loading and normalization | Media access/verification, deployment, devices; schema alone is not full validation |
...
| Pitch and scalar controls | Tunings, scales, shapes, envelopes, LFOs, modulation, timeline automation | Exact arithmetic where possible, scalar evaluation, envelope/LFO event state, route and binding conversion | Audio-rate realization, smoothing, antialiasing and host scheduling |
