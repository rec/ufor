...
`Computed.denominator_limit` is a maximum rational denominator, matching Tuney's actual
...
Ufor does not yet provide an MTS byte decoder or sparse update score.

## Oscillator contract

`Oscillator` carries `waveform` (sine, square, triangle), `duty_cycle` in [0,1],
`key_scale_note`, and `key_scale_db_per_12_steps` in dB per twelve note steps. Gain is
`10^(key_scale_db_per_12_steps*(note-key_scale_note)/240)`. The twelve-step gain unit is an
...
`SynthInstrumentScore` is the definition-only sibling of the sample instrument
score. Its body has declared controls, sustain, articulations, voice policy, and
one or more mapped `SynthVoice` templates. A template has an `Oscillator`, a
complete voice envelope and other shared sound settings, explicit mono-to-output
channel routes, and the same trigger/choke/articulation eligibility vocabulary as
sample slots. It intentionally has no assets, slices, loops, playback traversal,
processor graph, buffer API, or host MIDI binding.
...
`synth_instrument` score kinds in addition to recording, sequence, arrangement,
and sample instruments. Each has a `body` containing its definition. The initial
extraction deliberately retains `format = "recs"`
and `version = 3`; renaming the wire marker is a later explicit migration.

`schema/scores.json` is generated by `ufor.codec.score_schema`.
...
`score_toml` validates the complete current definition before serialization;
